...
test("Functional test for get profile details 1", async () => {
  axios.get.mockResolvedValue(fakeShowProfile);
  render(<BasicTabs user={"100"} />);
  const book = await screen.findByText("یادداشت‌های یک دیوانه");
  expect(book).toBeInTheDocument;
});

test("Functional test for get profile details 2", async () => {
  axios.get.mockResolvedValue(fakeShowProfile);
  render(<BasicTabs user={"100"} />);
  const book = await screen.findByText("خنده در تاریکی");
  expect(book).toBeInTheDocument;
});
...
test("Functional test for get profile details 2", async () => {
  axios.get.mockResolvedValue(fakeShowProfile);
  render(<BasicTabs user={"100"} />);
  const book = await screen.findByText("ولادیمیر ناباکوف");
  expect(book).toBeInTheDocument;
});
...
test("Functional test for get profile details 2", async () => {
  axios.get.mockResolvedValue(fakeShowProfile);
  render(<BasicTabs user={"100"} />);
  const book = await screen.findByText("نیکلای گوگول");
  expect(book).toBeInTheDocument;
});